...
- `scripts/game_state.gd` = 오토로드 싱글턴 `GameState`: 화면 간 선택 정보 전달. `CHARACTERS`(`Characters.names()` 5종 — 사본을 두지 않고 캐릭터 표에서 만든다), `MAPS`("랜덤" + `Maps.names()` 4종 — 마찬가지), `p1_config`/`p2_config`(character), `map_name`, `get_config(prefix)`. **무기 목록은 여기 없다**(이슈 #205) — 무기는 대기실이 아니라 라운드가 시작될 때 고르고, 후보는 서버가 `Weapons.random_choices()`로 직접 뽑는다.

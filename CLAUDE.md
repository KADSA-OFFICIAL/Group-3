...
- `scenes/select.tscn` + `scripts/select.gd` = 대기실 겸 무기 선택. `P1Panel`/`P2Panel`은 흰 카드(`Card`) 위에 얹히며 `Lobby.order` 슬롯에 대응하고 **자기 슬롯만 조작 가능**하고 상대 패널은 서버가 보낸 값을 표시만 한다. `StatusLabel`에 "상대 대기 중" 또는 양쪽 준비 상태, `GoButton`은 준비 토글. **씬 전환은 클라이언트가 스스로 하지 않고 `Lobby.match_starting`(서버 지시)을 받아서 한다.** 좌우 화살표는 **자기 맵 선택만** 바꾼다(`Lobby.submit_map()`) — `MapBox`에 양쪽 선택이 나란히 보이고, 실제로 쓸 맵은 시작할 때 서버가 둘 중 하나를 뽑는다. 그 아래 `WeaponBox`는 **고르는 곳이 아니라 안내판이다**(이슈 #205) — 무기는 라운드가 시작될 때 전투 화면에서 고른다고 적혀 있다.
  - **맵 칸(`MapBox`)은 양쪽이 고른 맵을 그림으로 보여준다**(이슈 #289). `ScreenRect`(진한 둥근 판) 안쪽에 `Map1`·`Map2`(`map_preview.gd`)가 좌우 절반씩 들어가 각자의 맵 배경 원화를 깔고, 이름표(`Map1Name`·`Map2Name`)도 각 절반 아래에 하나씩 있다 — **왼쪽이 1P, 오른쪽이 2P**이고 그림과 이름의 좌우가 어긋나면 누구 것인지 알 수 없다. 가운데 `QBadge`의 `?`는 실제 맵을 서버가 둘 중 하나에서 뽑는다는 표시이고, 안내 문구(`HintLabel`)는 **원화 위가 아니라 흰 카드 쪽으로 내렸다**(#112 — 밝은 맵 원화 위의 흰 글자는 아무리 덮어도 3:1을 못 넘긴다).
...
- `assets/fonts/Jalnan2.ttf` = **프로젝트 전역 한글 폰트**(잘난체, 이슈 #293). `project.godot`의 `gui/theme/custom_font`와 `resources/ui_theme.tres`의 `default_font`가 이 파일을 직접 가리킨다. 전에는 `resources/korean_font.tres`(SystemFont, OS 설치 글꼴 중 `Apple SD Gothic Neo`·`Malgun Gothic`·`Noto Sans CJK KR`·`NanumGothic`·`AppleGothic` 순으로 폴백)를 썼는데, OS마다 실제로 골라지는 글꼴이 달라 같은 글자 크기·같은 레이아웃에서도 글자 폭·줄 높이가 플랫폼마다 어긋났다(이슈 #290, `select.tscn`의 `WeaponBox` 문구가 Windows에서 상자 밖으로 넘쳤다). 번들 폰트로 바꾼 뒤에는 어떤 OS에서 실행하든 같은 파일로 렌더링되어 그 종류의 불일치가 구조적으로 없어진다. **새 폰트를 또 바꿀 때도 SystemFont로 되돌리지 말 것** — 다시 플랫폼마다 다른 렌더링이 재현된다.

...
- `scripts/maps.gd`(`class_name Maps`) = **맵 표 5종**(바다·화산·협곡·오두막·투기장). 이름과 씬 경로의 유일한 출처. `RANDOM` 상수와 `resolve()`(서버 전용 랜덤 확정)가 무기 표와 같은 형태다. **맵 씬 계약**: 루트 `Node2D`, `Spawns/Spawn1`·`Spawn2`(Marker2D, 순서가 1P·2P), 지형은 `StaticBody2D` + `CollisionShape2D`, 즉사 구역은 `Hazard`(Area2D), 배경도 맵이 그린다. 좌우 벽이 없는 맵은 화면 밖으로 나가면 낙사한다.

...
- `scripts/characters.gd`(`class_name Characters`) = **캐릭터 표 5종**(분홍·파랑·초록·노랑·빨강). 이름과 그림 경로의 유일한 출처이며 대기실 선택지·서버 검증·전투 화면 그림이 모두 여기서 나온다. 그림은 `assets/characters/`에 있고, 파일이 없으면 표의 몸통 색 단색으로 대신 그린다. 여백 측정은 `Art.content_rect()`가 한다.
  - **캐릭터마다 포즈가 셋이다**(이슈 #176) — 평소(`file`)·승리(`win_file`)·패배(`lose_file`). `pose_texture(name, pose)`가 골라 주고 그 포즈의 파일이 없으면 조용히 평소 그림으로 되돌아간다(무기의 `preview_file`과 같은 방식). 포즈 이름 상수는 `POSE_IDLE`·`POSE_WIN`·`POSE_LOSE`이고 **네트워크로 오가는 값**이다. 쓰는 곳은 전투 화면뿐이다 — 대기실·타이틀 미리보기와 결과 화면은 평소 그림을 쓴다.

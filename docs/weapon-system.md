...
- 무기 그래픽 — 7종(검·단검·전기톱·망치·대포 총·폭탄·활)만 그림이 있습니다.
  나머지 10종은 임시 막대이고 길이가 사거리, 색이 특수 쿨타임 상태입니다
- **광선검 그림(`laser_sword.png`)은 자리만 잡혀 있고 파일이 아직 없습니다** (이슈 #101).
  표에 `"file": "laser_sword.png"`가 들어가 있어서 `assets/weapons/`에 넣기만 하면 바로 나옵니다.
  파일이 없는 동안은 `Weapons.texture()`가 `null`을 돌려줘 임시 막대로 그려집니다 —
  넣을 그림은 **투명 배경**이어야 합니다. 흰 배경이면 흰 사각형 판으로 보입니다

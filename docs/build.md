...
젤리 워즈는 한 기기에서 둘이 하는 오프라인 게임이라(이슈 #320) 접속도 서버도 관전 빌드도
없습니다. **주 플랫폼은 모바일**이고(이슈 #322), PC 빌드는 키보드로 확인하는 용도로 남겨 둡니다.

| 프리셋 | 산출물 | 쓰임 |
| --- | --- | --- |
| `Android` | `build/JellyWars.apk` | 실제로 하는 것 — 화면 조이스틱 + 궁극기 버튼 |
| `Windows Desktop` | `build/JellyWars.exe` | 키보드로 확인 |
...
## Android 로 내보내기 (이슈 #322)

**처음 한 번은 준비가 더 필요합니다.** Windows 템플릿과 달리 Android 는 에디터 바깥의 도구가
있어야 합니다 — 에디터가 `편집기 설정 → 내보내기 → Android` 에서 이 셋의 경로를 묻습니다.

1. **Android SDK** (Android Studio 를 깔면 같이 들어옵니다)
2. **JDK 17**
3. **디버그 키스토어** — 에디터가 만들어 주거나, 직접 만든 것을 지정합니다

준비가 끝나면 `프로젝트 → 내보내기` 에서 `Android` 프리셋을 고르고 `build/JellyWars.apk` 로
내보냅니다. 명령줄로도 됩니다.

```bash
"Godot_v4.6.2-stable_win64_console.exe" --headless --path . --export-release "Android" build/JellyWars.apk
```

받은 사람은 apk 를 폰에 옮겨 설치합니다(스토어를 거치지 않으므로 **출처를 알 수 없는 앱 설치**를
한 번 허용해야 합니다). **기기를 가로로 놓고** 왼쪽 아래 조이스틱과 궁극기 버튼이 1P,
오른쪽 아래가 2P 입니다 — 화면 방향은 `project.godot` 에서 가로로 고정해 두었습니다.

> 이 개발 환경(Mac)에는 내보내기 템플릿도 Android SDK 도 없어 **실제 apk 를 만들어 보지
> 못했습니다.** 프리셋이 파싱되는 것까지만 확인했습니다.

...
## ## 맵은 라운드마다 새로 뽑는다
##
## 고르는 자리가 없다 (요청). 선택 창에서 사람마다 하나씩 골라 시작할 때 둘 중 하나를
## 뽑던 것을 없앴고, 지금은 라운드가 열릴 때 `main.gd` 가 직접 뽑는다
## (`main.gd` 의 `_pick_round_map()` → `_start_round` → `_receive_round_map`).
## 무기(#205)가 간 길과 같다 — 판마다 정해지는 것은 판이 열릴 때 정한다.
##
## **뽑기는 이 표를 섞어 만든 가방에서 한 장씩 꺼내는 방식이다** (#310) — 판마다 지형이
## 반드시 바뀌고, 이 표의 맵 수만큼 이어 하면 전부 한 번씩 지난다. 가방을 들고 있는
## 것은 이 표가 아니라 `main.gd` 다: 표는 상태 없이 목록만 준다.
##
## 그래서 **이 표에 맵을 하나 넣으면 그 순간부터 뽑기에 들어간다**. 선택 UI 를 손댈
## 일도, 이름을 다른 목록에 옮겨 적을 일도 없다 — 가방도 다음에 채울 때 저절로 늘어난다.
##
## ## 맵 씬이 지켜야 할 것
##
## - 루트는 `Node2D`.
...
## - 즉사 구역(물·용암)이 있으면 `Hazard`(Area2D). 닿으면 `main.gd` 가 죽인다.
...
## 맵 한 장이 그려지는 칸의 크기. 배경 원화(1920x1080)에 `scale 0.6` 을 곱한 값이고,
## 충돌 상자 좌표도 이 칸 안에서 잡혀 있다.
##
## **화면 크기와 같지 않다** (#329). 비율이 `expand` 라 뷰포트는 기기를 따라가지만
## (20:9 폰이면 1440x648) 맵은 여전히 이 칸에만 그려진다 — 원화만 키우면 그림의
## 발판과 실제 발판이 어긋나기 때문이다. 남는 자리를 어떻게 하는지는
## `main.gd` 의 `_center_map()` 과 `map_backdrop.gd` 에 있다.
const SIZE := Vector2(1152, 648)

## 실제 맵이 아닌 특수값. `main.gd` 가 실제 맵 하나로 확정한다 (resolve 참고).
const RANDOM := "랜덤"
...
## "랜덤"을 실제 맵 이름으로 바꾼다.
## **부르는 자리는 `main.gd` 하나다** — 뽑기가 여러 곳에서 돌면 가방을 나눠 갖게 된다.
##
## **라운드 뽑기는 이것을 쓰지 않는다** (#310). 판마다 다른 맵을 내려면 직전에 무엇이
## 나왔는지를 기억해야 하는데, 이 표는 상태를 들지 않는다 — 가방은 `main.gd` 가 들고 있다
## (`main.gd` 의 `_pick_round_map()`). 이 함수는 "랜덤"이라는 값을 실제 이름으로
## 확정해야 하는 다른 자리를 위해 남겨 둔 것이다.

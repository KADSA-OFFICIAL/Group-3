...
## main.gd 가 발동을 정한 뒤 화면에 띄우고(`main.gd`의 `_play_heavy_punch`),
...
var charged := false
## **예고 모드** (#231). 참이면 주먹이 아니라 "곧 여기 맞는다"만 그린다 —
## 테두리와, `preview_time`에 걸쳐 차오르는 안쪽이다. 갈래·주먹·결정은 안 그린다.
var preview := false
## 예고가 차는 데 걸리는 시간(초). 무기 표의 `punch_windup`이 그대로 들어온다.
var preview_time := 0.2
...
func _ready() -> void:
	# 예고는 테두리와 차오르는 안쪽뿐이라 뽑아 둘 것이 없다.
	if preview:
		return
...
func _wedge(radius: float) -> PackedVector2Array:
	...
	for i in 17:
		var angle := facing + lerpf(-half, half, float(i) / 16.0)

...
## 헤드리스 서버는 `--port=7778` 처럼 포트를 지정해 실행한다.
...
## 방을 따로 고르지 않았을 때 쓰는 포트 (= 1번 방).
const PORT := 7777

var is_server := false
## 이 프로세스가 실제로 쓰고 있는 포트. 서버면 열어둔 포트, 클라이언트면 접속한 포트.
var port := PORT
...
func _ready() -> void:
	...
	if should_run_as_server():
		start_server(port_from_cmdline())
...
func start_server(target_port: int = ROOMS[0]["port"]) -> Error:
	...
	if err != OK:
		push_error("서버 시작 실패 (포트 %d): %s" % [target_port, error_string(err)])
		return err

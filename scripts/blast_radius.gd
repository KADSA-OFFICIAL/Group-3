...
##
## 폭탄 그림 뒤에 깔리는 것은 **형제 순서**로 한다 — 씬에서 `Visual`·`ArtSprite`보다 앞에
## 있어서 같은 z 레이어 안에서 먼저 그려진다. **`z_index`를 내리면 안 된다**(이슈 #146):
## z를 내리면 z 0에 있는 것 **전부보다** 먼저 그려지는데, 맵이 자기 배경을 z 0의 불투명
## `ColorRect`로 깔기 때문에 원이 그 아래로 사라진다. `z_index = -1`로 넣었다가 네 맵
## 모두에서 아무것도 안 보였다. 올리는 것(`Effects`의 50)은 안전하고 내리는 것만 위험하다.
##
## 순수 표시라 판정과는 무관하다 — 실제 판정은 `projectile.gd`의 `_explode()`가 한다.

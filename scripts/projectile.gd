...
## 허공을 나는 것은 공유 무적을 타지 않는다 (확정). 항상 "projectile" 이다.
const SOURCE := "projectile"
## 이 슬롯의 젤리를 자동으로 따라간다 (단검). 0 이면 직선.
var homing_slot := 0
## 이 시간이 지나면 스스로 터진다 (폭탄). 0 이면 안 터진다.
var fuse := 0.0
## 터질 때 이 반경 안을 때린다 (폭탄). 0 이면 단발 명중.
var explosion_radius := 0.0
## 바닥에 남았을 때 이 슬롯의 주인이 주울 수 있다 (단검).
var pickup_owner := 0
...
func setup(data: Dictionary) -> void:
	shooter_slot = data["shooter_slot"]
	damage = data["damage"]
	...
	homing_slot = data.get("homing_slot", 0)
...
func _physics_process(delta: float) -> void:
	...
	if _landed:
		if pickup_owner != 0:
			...
			if owner_jelly != null and position.distance_to(owner_jelly.global_position) <= 48.0:
				_done = true
				picked_up.emit(pickup_owner, name)
		return

	# 유도 (단검) — 상대를 향해 방향을 계속 고친다.
	if homing_slot != 0:
		var target := _find_jelly(homing_slot)
		if target != null:
...
func _find_jelly(slot: int) -> Node:
...
func _explode() -> void:
	if _done:
		return
	for jelly: Node in get_tree().get_nodes_in_group("jellies"):
		if jelly.slot == shooter_slot or not jelly.alive:
			continue

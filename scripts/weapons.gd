...
##   spin_time       특수를 누르고 나서 **제자리에서 무기를 돌리는** 시간(초) (전기톱, #260).
##                   위의 special_windup 과 뜻이 겹쳐 보이지만 다른 것이다 — 그쪽은
##                   조작을 막지 않는 예고고, 이쪽은 강제 이동 상태("spin")라 그동안
##                   움직이지도 때리지도 못한다. 끝나면 돌진("dash")으로 이어진다
##   bleed_interval  출혈이 들어가는 간격(초). 없으면 1.0.
##                   한 틱은 bleed_damage * bleed_interval 이라 **간격을 바꿔도
##                   총량(bleed_damage * bleed_duration)은 그대로다** — 촘촘하게
##                   나눌수록 "계속 깎이는" 것으로 보인다 (main.gd._tick_bleeds 참고)
...
##   smoke_puffs     무기에서 피어오르는 연기 덩어리 수. 없거나 0이면 연기가 없다.
##                   기본은 위 두 그림 중 하나가 떠 있는 동안만 난다
##                   (Player._update_weapon_smoke 참고)
##   smoke_fuse      연기가 **그림과 무관하게 늘** 나고 색이 검다 (폭탄의 도화선).
##                   smoke_puffs 와 함께 쓴다 — 이 값만으로는 연기가 나지 않는다
...
##   throw_spin      던진 무기 그림이 날아가는 내내 도는 속도(초당 라디안, 방패).
##                   0이거나 없으면 지금까지처럼 안 돈다. 도는 쪽은 날아가는 쪽을 따른다 —
##                   진행 방향으로 한 번 돌려 굳히는 것과 함께 쓸 수 없다
##                   (Projectile.art_spin 참고)
...
##   projectile_orb   이 무기가 쏘는 탄을 파란 에너지 구슬로 그린다 (기본·특수 모두).
##                    projectile_arrow 와 같은 자리의 값이다 — 그림 파일 없이
##                    Projectile._draw()가 그린다. 미사일(special_missile)이 켜진
##                    탄은 그쪽이 이긴다: 특수는 미사일, 기본은 구슬로 나간다
##   projectile_hit_burst 이 무기의 탄이 맞은 자리에 푸른 충격 파열을 띄운다.
##                    hit_sparks(단검의 빨간 알갱이)와 같은 자리의 값이다 —
##                    연출만 붙고 데미지·넉백은 하나도 안 변한다
##   basic_tick_sfx   기본 공격의 **데미지가 들어갈 때마다** 짧은 틱 소리를 낸다.
##                    지속 데미지 무기(광선검·전기톱, 0.2초 간격)에 붙여 "따다다닥"으로
##                    들리게 하는 값이다. 이 값이 있으면 공용 피격음은 나지 않는다 —
##                    0.2초마다 둘이 겹치면 리듬이 아니라 웅웅거림이 된다
##   special_tick_sfx 위와 같지만 **특수로 들어가는 데미지**에만 붙는다 (소총 연사).
##                    special_missile 이 특수 전용인 것과 같은 갈래다
...
const LIST: Array[Dictionary] = [
	...
	},
	{
		...
		"trail_ghosts": 5,
		# 데미지가 0.2초마다 들어가는 무기인데 소리는 공용 피격음 하나로 0.6초에 한 번만
		# 났다 — 닿아 있는 동안 무엇이 얼마나 들어가는지 귀로 알 수 없었다.
		# 짧은 틱(0.12초)을 매 데미지마다 내서 초당 다섯 번 "따다다닥"으로 들리게 한다.
		"basic_tick_sfx": true,
	},
	{
		"name": "전기톱",
		...
		# **누르면 곧바로 튀어 나가지 않는다** (#260). 이 시간 동안 제자리에 서서
		# 톱을 돌리고("spin"), 그 뒤에 지금까지와 똑같은 돌진("dash")으로 이어진다.
		# 데미지는 돌진 구간에만 들어가므로(예약의 `modes`) 도는 동안은 안 맞는다.
		#
		# 0.45초는 **상대가 반응할 수 있는 가장 짧은 예고**로 잡았다 — 강펀치 예고가
		# 0.2 → 0.3 → 0.5초로 두 번 늘어난 끝에 자리를 잡았고(#235·#238), 그것이
		# 사람이 보고 비키는 데 필요한 시간의 대역이다. 여기서 더 줄이면 예고가
		# 예고로 안 읽히고, 더 늘리면 쿨타임 7초짜리가 거의 안 맞는 기술이 된다.
		"spin_time": 0.45,
		# 3초 동안 총 12. **간격만 0.2초로 촘촘하게 바꿨다** (#260) — 한 틱은
		# 4.0 * 0.2 = 0.8 이고 15번 들어가서 총량은 전과 같다. 같은 무기의 기본
		# 공격이 이미 0.2초 박자인데(#105) 출혈만 1초에 한 번 크게 들어와서,
		# 지속 피해인 것이 화면에도 귀에도 안 읽혔다.
		"bleed_damage": 4.0, "bleed_duration": 3.0, "bleed_interval": 0.2,
		# 광선검과 같은 이유·같은 값이다 — 0.2초마다 들어가는 지속 데미지에 틱을 붙인다.
		"basic_tick_sfx": true,
	},
	{
		"name": "망치",
		...
		# **특수는 때리지 않는다 — 능력을 건다** (광선검의 관통과 같은 갈래).
		# `special_duration` 동안 기본 공격에 `stun_duration` 짜리 기절이 얹힌다.
		#
		# 전에는 `special_damage` 16 + 넉백 + 1.2초 기절이 한 번 들어가는 즉시 판정이었다.
		# 그런데 이 무기의 `desc`("스킬 사용 시, 공격마다 적에게 '기절' 효과를 부여합니다")와
		# `special`("닿으면 일정 시간 피격 시 기절 효과 부여")은 **처음부터 지금 이 동작을
		# 적고 있었다** — 구현이 문서를 따라가지 못한 쪽이었다.
		"special_damage": 0.0, "special_cooldown": 8.0, "knockback": 2,
		"special_duration": 4.0,
		# 1.2 → **0.3**. 한 번만 걸던 기절이 4초 동안 매 타(0.9초 간격이라 4~5타)에
		# 얹히므로, 같은 길이를 두면 그 4초가 통째로 기절이 된다 — 걸린 쪽이 아무것도
		# 못 하는 시간을 만들면 안 된다. 0.3초면 얻어맞는 박자마다 짧게 굳는다.
		"stun_duration": 0.3,
	},
	{
		"name": "대포 총",
		...
		#
		# 1.5 → **2.0**. 막대를 파란 구슬로 바꾸면서(아래 projectile_orb) 같이 올렸다 —
		# 구슬은 가로 18px 막대와 달리 지름이 곧 크기라, 1.5배로는 노란 막대보다
		# 오히려 작게 읽혔다. 충돌 상자도 함께 커진다(`_apply_size`)는 점을 적어 둔다:
		# 눈에만 크게 하려는 것이 아니라 **닿는 범위도 그만큼 커진다**.
		"projectile_scale": 2.0,
		# 탄을 **파란 동그라미**로 그린다. 공용 노란 막대(18×6)는 아홉 무기가 같이 쓰는
		# 그림이라 "대포에서 나온 포탄"으로 읽히지 않았다 — 활이 결정질 화살로
		# 갈라진 것(projectile_arrow, #125)과 같은 갈래다.
		#
		# 특수는 이미 불꽃 꼬리 미사일이라 구슬을 쓰지 않는다(아래 special_missile) —
		# 이 값은 기본 공격 탄에만 보인다. 하나의 무기가 기본과 특수를 다른 모양으로
		# 내보내는 것은 지금까지의 규칙 그대로다.
		"projectile_orb": true,
		# 맞은 자리에서 푸른 충격이 터진다. 포탄 하나에 7 데미지가 조용히 들어가면
		# 맞았는지 안 맞았는지가 화면에 안 남는다 — 발사 간격이 1.4초로 느린 무기라
		# 한 발 한 발이 눈에 보여야 한다.
		"projectile_hit_burst": true,
	...
	},
	{
		...
		# 도화선이 늘 타고 있으므로 손에 든 폭탄에서 **검은 연기가 항상** 난다
		# (`smoke_fuse`). 표창·도끼의 붉은 연기와 달리 상태를 알리는 것이 아니라
		# 무기의 생김새다 — 강화 폭탄인지는 그림이 이미 말한다.
		"smoke_puffs": 4,
		"smoke_fuse": true,
	...
	},
	{
		...
		"hit_lightning": true,
		# 던진 삼지창 그림만 40px → 104px (#155). 원화가 1:3.96으로 가늘고 길어서
		# 기본 크기에서는 자루가 선 한 줄이 되고 갈래 셋이 뭉갠다. 2.0(80px)으로
		# 젤리 몸통(72px)만큼 키워 두었던 것을 한 번 더 키운 값이다 — 몸통보다 길어야
		# "던진 무기"로 읽히고, 맞으면 번개가 내려치는 한 방짜리 특수에 걸맞다.
		# 판정은 하나도 안 바뀐다 (`projectile_scale`이 아니라 그림만 키우는 값이다).
		"projectile_art_scale": 2.6,
	},
	{
		"name": "글러브",
	...
	},
	{
		...
		# **예전에는 데미지가 0이었다**(요청으로 14로 되돌림). 맞아도 아프지 않으니
		# 빨간 표창이 나오면 오히려 안심하고 맞으러 가는 판이 되었다 — 위치 교환이
		# 벌이 아니라 상이 되어 버린 것이다. 지금은 일반 표창과 **같은 14**를 주고,
		# 위치 교환은 그 위에 얹는다. 세게 느껴지면 이 숫자만 내린다(넉백은 계속 0 —
		# 자리를 바꾼 직후에 또 밀어내면 어디로 갔는지를 눈으로 못 쫓는다).
		# 확률은 기획서의 15% → 35%로 올렸다(요청). 쿨타임 3초에 15%면 라운드 내내 한 번도
		# 안 나오기 쉬워서, 있는지 없는지 모르는 기능이 된다. 35%면 던지기 3번에
		# 한 번쯤이다. **아직 플레이로 확인한 값은 아니다** — 세게 느껴지면 이 숫자만 줄인다.
		"empowered_chance": 0.35, "empowered_damage": 14.0, "empowered_knockback": 0,
		# 그림이 따로다 — 맞으면 데미지에 더해 위치까지 바뀌는 것이라
	...
	},
	{
		...
		# 연사로 들어가는 데미지마다 짧은 틱을 낸다 (광선검·전기톱과 같은 소리).
		# **기본 공격에는 안 붙는다** — 그쪽은 1.2초에 한 발이라 따다다닥이 될 수 없고,
		# 공용 피격음이 더 어울린다. 붙는 곳이 특수뿐이라 `basic_` 이 아니라 `special_` 이다.
		"special_tick_sfx": true,
	},
	{
		"name": "방패",
		...
		# 세워 둔 채 날리면 손에 든 모습 그대로 미끄러져 가서 던진 것으로 안 보였다 —
		# 도는 동안 옆으로 눕는 순간이 계속 지나가는 것이 원반의 모습이다.
		# `art_upright`(폭탄)와 `_face()`(진행 방향으로 굳히기) 둘 다 쓰지 않는다.
		"throw_spin": 15.0,
		# 던진 방패 그림만 40px → 72px. 원화가 0.815:1로 뭉툭해서 기본 크기에서는
		# 돌아가는 작은 조각으로 보였다 — 넓게 막는 물건을 던진 것이라 젤리 몸통(72px)
		# 만큼은 되어야 방패로 읽힌다. 삼지창(#155)과 같은 값이고 판정은 안 바뀐다.
		"projectile_art_scale": 1.8,
	},
]

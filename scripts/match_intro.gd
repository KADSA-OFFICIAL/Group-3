...
##
## 화면에 띄우는 일만 한다. 언제 띄울지도, 그동안 무기 선택을 미루는 것도 `main.gd` 가
## 정한다 (`main.gd`의 `_start_round` → `_play_match_intro`) — 무기 선택 카드
## (`weapon_pick.gd`)와 같은 짜임이다.

## 처음부터 끝까지 걸리는 시간(초). **`main.gd` 의 `MATCH_INTRO_TIME`과 같아야 한다** —
## `main.gd` 는 이 시간만큼 무기 선택을 미루므로, 여기가 더 길면 그림이 덜 사라진 채로
